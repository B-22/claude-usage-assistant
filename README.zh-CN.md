...
(卡片只显示用量百分比和 MAX/PRO 角标,不显示邮箱/账号;但请确认截图里其它窗口也没有泄露个人信息。) -->
...
- **菜单**(右键或 `☰` 按钮):可选**卡片显示哪些窗口**、**托盘显示哪个窗口**,以及缩放、不透明度、重置显示方式(时刻 / 倒计时)。
...
- **顶栏:** `▲` 置顶 · `☰` 菜单 · `↻` 刷新 · `✕` 退出。
- **菜单**(右键或 `☰`):立即刷新 · 卡片显示 · 托盘显示 · 缩放 · 不透明度 · 重置显示 · 窗口置顶 · 隐藏到托盘 · 退出。
